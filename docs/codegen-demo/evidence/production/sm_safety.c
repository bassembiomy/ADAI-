...
static const SM_Node_t SM_State_Parent_Map[SM_NUM_STATES + 1U] = {
    [0] = SM_NODE_INVALID,
    [SM_ST_HEATER_IDX] = SM_NODE_INVALID,
    [SM_ST_STANDBY_IDX] = SM_ST_HEATER,
    [SM_ST_WARMING_IDX] = SM_ST_HEATER,
    [SM_ST_PWM_BANK_IDX] = SM_ST_WARMING,
    [SM_ST_PWM_LOW_IDX] = SM_ST_PWM_BANK,
    [SM_ST_PWM_HIGH_IDX] = SM_ST_PWM_BANK,
    [SM_ST_FAN_BANK_IDX] = SM_ST_WARMING,
    [SM_ST_FAN_LOW_IDX] = SM_ST_FAN_BANK,
    [SM_ST_FAN_HIGH_IDX] = SM_ST_FAN_BANK,
    [SM_ST_OVERHEAT_IDX] = SM_NODE_INVALID,
};

static const bool SM_Layer_Has_Children_Map[SM_NUM_LAYERS] = {
    [SM_LYR_ROOT_IDX] = true,
    [SM_LYR_HEATER_CHILDREN_IDX] = true,
    [SM_LYR_PWM_CHILDREN_IDX] = true,
    [SM_LYR_FAN_CHILDREN_IDX] = true,
    [SM_LYR_WARMING_REGIONS_IDX] = true,
};
...
static bool SM_Is_Direct_Layer_Child(uint32_t layer_index, SM_Node_t state)
{
    switch (layer_index) {
        case SM_LYR_ROOT_IDX:
            switch (state) {
                case SM_ST_HEATER: return true;
                case SM_ST_OVERHEAT: return true;
                default: return false;
            }
        case SM_LYR_HEATER_CHILDREN_IDX:
            switch (state) {
                case SM_ST_STANDBY: return true;
                case SM_ST_WARMING: return true;
                default: return false;
            }
        case SM_LYR_PWM_CHILDREN_IDX:
            switch (state) {
                case SM_ST_PWM_LOW: return true;
                case SM_ST_PWM_HIGH: return true;
                default: return false;
            }
        case SM_LYR_FAN_CHILDREN_IDX:
            switch (state) {
                case SM_ST_FAN_LOW: return true;
                case SM_ST_FAN_HIGH: return true;
                default: return false;
            }
        case SM_LYR_WARMING_REGIONS_IDX:
            switch (state) {
                case SM_ST_PWM_BANK: return true;
                case SM_ST_FAN_BANK: return true;
                default: return false;
            }
        default: return false;
    }
