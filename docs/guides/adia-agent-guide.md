...
## 4. Supported Engineering Workflows
...
### X-BRIDGES BLDC Motor Drive
- **Prompt:** *"Create an X-BRIDGES model for a BLDC motor with three-phase inverter."*
- **Catalog Verification:** Verifies all requested blocks against the live catalog:
  - `bldc_motor`
  - `three_phase_inverter`
  - `dc_voltage_source`
  - `hall_sensor` / `current_sensor`
  - `pwm_controller`
- **Port Compatibility Check:** Verifies electrical, mechanical, and signal port matches before proposing connections.
...
- **Tabs:**
  - **Workflow:** Conversational prompt, clarification Q&A, and interactive approval cards.
