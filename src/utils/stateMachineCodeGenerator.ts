...
const generateLegacyMISRACCode = (
  ...
  const isTerminalState = (s: StateData): boolean => {
    if (!s) return false;
    if ((s as any).type === 'end' || (s as any).isFinal === true || s.isTerminalState === true || s.isTerminal === true) return true;
  ...
  /* Post-processing: normalize generated C code and enforce brace balance. */
  const validateGeneratedCode = (code: string): string => {
    ...
    if (depth > 0) {
      const endifPos = result.lastIndexOf('#endif');
      if (endifPos !== -1) {
        const prefix = result.substring(0, endifPos).trimEnd();
  ...
  const blockStates: { declaration: string; member: string; initialValue: number }[] = [];
      ...
      s.xBridgesModel.nodes.forEach(n => {
        if (['Integrator', 'INTEGRATOR_CONTINUOUS', 'DELAY'].includes((n.data as any).type)) {
          ...
          if (!seenBlockMembers.has(member)) {
            ...
            const ic = Number((n.data as any).params?.initial_condition ?? 0);
            blockStates.push({
              declaration: `    volatile float ${member};`,
              member,
              initialValue: isFinite(ic) ? ic : 0,
            });
          }
        }
      });
...
${blockStates.length > 0 ? blockStates.map(bs => bs.declaration).join('\n') + '\n' : ''}} SM_Data_t;
...
static inline uint32_t HAL_GetTick(void) { return 0; }
  ...
  /* Post-generation structural guard: reject any C/H file with unbalanced
   * braces before it can be handed off for compilation. */
  const checkBraceBalance = (code: string): boolean => {
    let depth = 0;
    let inString = false;
    let strChar = '';
    let inBlockComment = false;
  ...
  baseFiles.forEach(f => {
    if ((f.name.endsWith('.c') || f.name.endsWith('.h')) && !checkBraceBalance(f.content)) {
      errors.push({
        id: uuidv4(),
        type: 'error',
        message: `Generated file '${f.name}' has unbalanced braces. This is a generator bug — please report the model that produced it.`,
        timestamp: new Date(),
