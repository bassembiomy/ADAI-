...
const microwaveLab = {
  ...
  nodes: [
    ...
    { id: 'mw_cavity', blockId: 'microwave_cavity', position: { x: 700, y: 250 }, label: '55L Cavity', params: { volume: 55 } },
...
describe('Microwave 55L temperature behavior', () => {
  it('should not increase without limit', () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const tempC = state.scopeValues?.value ?? state.scopeValues;
      temps.push(tempC);
      times.push(state.time);
      if (i % 1 === 0) {
        console.log(`t=${state.time.toFixed(1)}s T=${tempC.toFixed(2)}°C`);
      }
      if (tempC > 300) {
        console.log('Temperature exceeded 300°C at t=', state.time);
        break;
      }
    }

    expect(Math.max(...temps)).toBeLessThan(100);
  }, 10000);
});
