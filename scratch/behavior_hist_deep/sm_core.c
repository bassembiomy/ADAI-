...
static void SM_Enter_Layer_1(ADIA_Instance_t* instance, bool use_history);
static void SM_Step_Layer_1(ADIA_Instance_t* instance, uint32_t delta_ms);
static void SM_Enter_Layer_2(ADIA_Instance_t* instance, bool use_history);
static void SM_Step_Layer_2(ADIA_Instance_t* instance, uint32_t delta_ms);
...
void SM_Reset(ADIA_Instance_t* instance) {
    ...
    instance->error_status = SM_ERR_NONE;
    SM_Enter_Layer_2(instance, false);
}
...
void SM_Step(ADIA_Instance_t* instance, uint32_t delta_ms) {
    ...
    if (instance->state_active[3U]) {
        if (delta_ms > (UINT32_MAX - instance->state_timers[3U])) {
            instance->state_timers[3U] = UINT32_MAX;
        } else {
            instance->state_timers[3U] += delta_ms;
        }
    }
    if (instance->state_active[4U]) {
        if (delta_ms > (UINT32_MAX - instance->state_timers[4U])) {
            instance->state_timers[4U] = UINT32_MAX;
        } else {
            instance->state_timers[4U] += delta_ms;
        }
    }
    if (instance->state_active[5U]) {
        if (delta_ms > (UINT32_MAX - instance->state_timers[5U])) {
            instance->state_timers[5U] = UINT32_MAX;
        } else {
            instance->state_timers[5U] += delta_ms;
        }
    }

    /* Step root layer */
    SM_Step_Layer_2(instance, delta_ms);
}
    ...
    switch (state) {
        case SM_ST_A:
            if (instance->active_states[0U] != SM_NODE_INVALID) {
                SM_Exit_State(instance, instance->active_states[0U]);
            }
            SM_ST_A_Exit(instance);
            instance->state_active[0U] = false;
            instance->state_timers[0U] = 0U;
            instance->history_states[1U] = state;
        ...
        case SM_ST_P:
            if (instance->active_states[1U] != SM_NODE_INVALID) {
                SM_Exit_State(instance, instance->active_states[1U]);
            }
            SM_ST_P_Exit(instance);
            instance->state_active[3U] = false;
            instance->state_timers[3U] = 0U;
            instance->active_states[2U] = SM_NODE_INVALID;
            break;
        case SM_ST_X:
            SM_ST_X_Exit(instance);
            instance->state_active[4U] = false;
            instance->state_timers[4U] = 0U;
            instance->history_states[0U] = state;
            instance->active_states[0U] = SM_NODE_INVALID;
            break;
        case SM_ST_Y:
            SM_ST_Y_Exit(instance);
            instance->state_active[5U] = false;
            instance->state_timers[5U] = 0U;
            instance->history_states[0U] = state;
            instance->active_states[0U] = SM_NODE_INVALID;
            break;
        case SM_NODE_SAFE:
...
static void SM_Enter_State_Shallow(ADIA_Instance_t* instance, SM_Node_t state) {
    switch (state) {
        case SM_ST_A:
            instance->active_states[1U] = state;
        ...
        case SM_ST_B:
            ...
            break;
        case SM_ST_OUT:
            instance->active_states[2U] = state;
            instance->state_active[2U] = true;
            instance->state_timers[2U] = 0U;
            SM_ST_OUT_Entry(instance);
            break;
        case SM_ST_P:
            instance->active_states[2U] = state;
            instance->state_active[3U] = true;
            instance->state_timers[3U] = 0U;
            SM_ST_P_Entry(instance);
            break;
        case SM_ST_X:
            instance->active_states[0U] = state;
            instance->state_active[4U] = true;
            instance->state_timers[4U] = 0U;
            SM_ST_X_Entry(instance);
            break;
        case SM_ST_Y:
            instance->active_states[0U] = state;
            instance->state_active[5U] = true;
            instance->state_timers[5U] = 0U;
            SM_ST_Y_Entry(instance);
            break;
        case SM_NODE_SAFE:
...
static void SM_Enter_State(ADIA_Instance_t* instance, SM_Node_t state, bool use_history) {
    ...
    switch (state) {
        case SM_ST_A:
            SM_Enter_Layer_0(instance, use_history);
            break;
        case SM_ST_P:
            SM_Enter_Layer_1(instance, use_history);
            break;
        default:
            break;
    }
...
static void SM_Enter_Layer_0(ADIA_Instance_t* instance, bool use_history) {
    if (use_history && (instance->history_states[0U] != SM_NODE_INVALID)) {
        ...
        SM_Enter_State(instance, SM_ST_X, false);
    }
}

/**
 * @brief Enters layer 1 and initializes default states or junctions.
 * @param instance Pointer to state machine context
 * @param use_history True to restore history states
 */
static void SM_Enter_Layer_1(ADIA_Instance_t* instance, bool use_history) {
    if (use_history && (instance->history_states[1U] != SM_NODE_INVALID)) {
        SM_Enter_State(instance, instance->history_states[1U], true);
    } else {
...
/**
 * @brief Enters layer 2 and initializes default states or junctions.
 * @param instance Pointer to state machine context
 * @param use_history True to restore history states
 */
static void SM_Enter_Layer_2(ADIA_Instance_t* instance, bool use_history) {
    if (use_history && (instance->history_states[2U] != SM_NODE_INVALID)) {
        SM_Enter_State(instance, instance->history_states[2U], false);
    } else {
        SM_Enter_State(instance, SM_ST_P, false);
    }
}
...
static void SM_Step_Layer_0(ADIA_Instance_t* instance, uint32_t delta_ms) {
    switch (instance->active_states[0U]) {
        case SM_ST_X:
            ...
            if ((instance->data.t1)) {
                SM_Exit_State(instance, SM_ST_X);
                SM_Enter_State(instance, SM_ST_Y, false);
                return;
            }
            else { /* MISRA 15.7 */ }
            /* Run During Actions */
            SM_ST_X_During(instance, delta_ms);
            break;
        case SM_ST_Y:
            /* Evaluate Outgoing Transitions */
            if ((instance->data.t2)) {
                SM_Exit_State(instance, SM_ST_Y);
                SM_Exit_State(instance, SM_ST_A);
                SM_Exit_State(instance, SM_ST_P);
                SM_Enter_State(instance, SM_ST_OUT, false);
                return;
            }
            else { /* MISRA 15.7 */ }
            /* Run During Actions */
            SM_ST_Y_During(instance, delta_ms);
            break;
        default:
            break;
    }
...
/**
 * @brief Evaluates transitions and executes during actions for layer 1.
 * @param instance Pointer to state machine context
 * @param delta_ms Execution tick period in milliseconds
 */
static void SM_Step_Layer_1(ADIA_Instance_t* instance, uint32_t delta_ms) {
...
/**
 * @brief Evaluates transitions and executes during actions for layer 2.
 * @param instance Pointer to state machine context
 * @param delta_ms Execution tick period in milliseconds
 */
static void SM_Step_Layer_2(ADIA_Instance_t* instance, uint32_t delta_ms) {
    switch (instance->active_states[2U]) {
        case SM_ST_P:
            /* Evaluate Outgoing Transitions */
            /* Run During Actions */
            SM_ST_P_During(instance, delta_ms);
            /* Step Child Layers */
            SM_Step_Layer_1(instance, delta_ms);
            break;
        case SM_ST_OUT:
            /* Evaluate Outgoing Transitions */
            if ((instance->data.t3)) {
                SM_Exit_State(instance, SM_ST_OUT);
                SM_Enter_State(instance, SM_ST_P, true);
                return;
            }
            else { /* MISRA 15.7 */ }
            /* Run During Actions */
            SM_ST_OUT_During(instance, delta_ms);
            break;
        default:
            break;
    }
