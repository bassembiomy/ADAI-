...
void SM_Init(ADIA_Instance_t* instance) {
    ...
    instance->data.counter = 0U;
    instance->data.is_active = false;
    instance->data.sensor_val = 0.0f;
...
void SM_Sync_IO(ADIA_Instance_t* instance) {
    ...
    instance->data.sensor_val = (float)MCAL_Dio_ReadChannel(MCAL_PIN_INPUT_0);

    /* SM-to-MCAL Output Signal Binding */
    MCAL_Dio_WriteChannel(MCAL_PIN_OUTPUT_0, instance->data.is_active);
...
static void SM_Enter_State_Shallow(ADIA_Instance_t* instance, SM_Node_t state) {
    switch (state) {
        case SM_ST_ACTIVE:
            ...
            instance->state_timers[0U] = 0U;
            SM_ST_ACTIVE_Entry(instance);
            break;
        case SM_ST_IDLE:
            instance->active_states[0U] = state;
            instance->state_active[1U] = true;
            instance->state_timers[1U] = 0U;
            SM_ST_IDLE_Entry(instance);
            break;
        case SM_NODE_SAFE:
...
static void SM_Enter_Layer_0(ADIA_Instance_t* instance, bool use_history) {
    if (use_history && (instance->history_states[0U] != SM_NODE_INVALID)) {
        ...
        SM_Enter_State(instance, SM_ST_IDLE, false);
    }
}
...
static void SM_Step_Layer_0(ADIA_Instance_t* instance, uint32_t delta_ms) {
    switch (instance->active_states[0U]) {
        case SM_ST_IDLE:
            ...
            if ((instance->data.sensor_val > 10.0f)) {
                SM_Exit_State(instance, SM_ST_IDLE);
                /* Action */
                instance->data.counter = (uint16_t)(5U);
                SM_Enter_State(instance, SM_ST_ACTIVE, false);
                return;
            }
            else { /* MISRA 15.7 */ }
            /* Run During Actions */
            SM_ST_IDLE_During(instance, delta_ms);
            break;
        case SM_ST_ACTIVE:
            /* Evaluate Outgoing Transitions */
            if ((instance->data.counter >= 100U)) {
                SM_Exit_State(instance, SM_ST_ACTIVE);
                SM_Enter_State(instance, SM_ST_IDLE, false);
                return;
            }
            else { /* MISRA 15.7 */ }
            /* Run During Actions */
            SM_ST_ACTIVE_During(instance, delta_ms);
            break;
        default:
            break;
    }
