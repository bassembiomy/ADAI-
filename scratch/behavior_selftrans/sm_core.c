...
static void SM_Enter_Layer_0(ADIA_Instance_t* instance, bool use_history) {
    if (use_history && (instance->history_states[0U] != SM_NODE_INVALID)) {
        ...
        SM_Enter_State(instance, SM_ST_S1, false);
    }
}
...
static void SM_Step_Layer_0(ADIA_Instance_t* instance, uint32_t delta_ms) {
    switch (instance->active_states[0U]) {
        case SM_ST_S1:
            ...
            if ((instance->data.t1)) {
                SM_Exit_State(instance, SM_ST_S1);
                SM_Enter_State(instance, SM_ST_S1, false);
                return;
            }
            else if ((instance->data.t2)) {
                SM_Exit_State(instance, SM_ST_S1);
                SM_Enter_State(instance, SM_ST_S2, false);
                return;
            }
            else { /* MISRA 15.7 */ }
            /* Run During Actions */
            SM_ST_S1_During(instance, delta_ms);
            break;
        case SM_ST_S2:
            /* Evaluate Outgoing Transitions */
            if ((instance->data.t3)) {
            ...
            /* Run During Actions */
            SM_ST_S2_During(instance, delta_ms);
            break;
        default:
            break;
    }
